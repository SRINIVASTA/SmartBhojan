...
st.markdown("""
    ...
    /* The Custom Frame that wraps the content */
    .smart-frame {
        ...
        margin-top: -60px; /* Pulls content up to top */
    }
    
    /* Smart Bhojan Title Styling */
    .smart-title {
        text-align: center;
        color: #2E7D32;
...
def load_data():
    ...
    # Ensure recipe column exists
    if 'Instructions' not in df.columns:
        df['Instructions'] = "Recipe instructions are currently unavailable."
    df.fillna({"Instructions": "No data.", "Protein (g)": 0, "Carbs (g)": 0, "Fat (g)": 0, "Calories (kcal)": 0}, inplace=True)
    return df
...
# --- MAIN UI (INSIDE THE BORDER) ---
# Start the frame
st.markdown('<div class="smart-frame">', unsafe_allow_html=True)
...
if food_choice and food_choice != "No results":
    ...
    with col2:
        ...
        st.pyplot(fig)
        st.info(f"💡 **Tip:** {tip}")
        st.download_button("📥 Download Smart Bhojan Report (PDF)", create_recipe_pdf(food_choice), f"SmartBhojan_{food_choice}.pdf", "application/pdf", use_container_width=True)
